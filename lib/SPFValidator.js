...
class SPFValidator {
  constructor(options, dnsCount) {
    ...
    this.DNS = new resolver.DNSResolver(options, dnsCount);
  ...
  validate(hostip, sender, spf, pResolve, pReject) {
    ...
    Promise.all(arr).then((values) => {
      ...
      if(me.redirect) {
        ...
        let child = new SPFValidator(childOpts, me.DNS.count);
  ...
  validateMechanism(hostip, sender, mechanism) {
    ...
    switch(mechanism.type) {
      ...
      case 'exp':
        ...
        try{
          this.macroIsValid(mechanism.value, true);
          if(this.exp !== null) {
            return Promise.resolve(PERMERROR);
          }
          this.exp = mechanism.value;
          return Promise.resolve(NEUTRAL);
        } catch(e) {
          if(e.name === 'SPFValidatorError') {
  ...
  macroIsValid(macro, isExp) {
    if(macro === undefined || this.domainIsValid(macro)) {
      throw new SPFValidatorError('Invalid Macro', 'PERMERROR');
    }
    if(isExp && macro.includes('%{r}')) {
      throw new SPFValidatorError('Invalid Macro', 'PERMERROR');
    }
    if(macro.includes('%')) {
      let index = macro.indexOf('%');
      if(macro[index+1] !== '{' && macro[index+1] !== '%' && 
         macro[index+1] !== '-' && macro[index+1] !== '_') {
        throw new SPFValidatorError('Invalid Macro', 'PERMERROR');
      }

      if(macro.match(/%{[^slodipvh]}/)) {
  ...
  getSubnets(value) {
    ...
    if(/\/\d{0,3}(\/)?\d/.test(value)) {
      let split = value.split('/');
      split[0] = this.macroIsValid(split[0]);
      if(split.length === 2) {
  ...
  validateA(hostip, sender, mechanism) {
    let domain = this.options.domain;
    let subnets = this.getSubnets(mechanism.value);
    ...
    return new Promise((resolve, reject) => {
      dnsPromise.then((records) => {
        ...
        for(let i = 0; i < records.length; i++) {
          let ipText = records[i]+'/'+subnets[0];
          if(ipv6) {
            ipText = records[i]+'/'+subnets[1];
          }
        ...
        reject(new SPFValidatorError(e.message, 'FAIL'));
      });
    });
  }

  validateMx(hostip, sender, mechanism) {
    let domain = this.options.domain;
    let subnets = this.getSubnets(mechanism.value);
    let domainValue = subnets.shift();
    if(domainValue !== null) {
  ...
  validateExists(hostip, sender, mechanism) {
    if(mechanism.value === undefined) {
      return Promise.resolve(PERMERROR);
    }
    this.macroIsValid(mechanism.value);
  ...
  validateInclude(hostip, sender, mechanism) {
    if(mechanism.value === undefined) {
      return Promise.resolve(PERMERROR);
    }
    if(this.domainIsValid(mechanism.value)) {
      throw new SPFValidatorError('Include domain is not valid', 'PERMERROR');
    }
    let opts = this.options;
    opts.isInclude = true;
    opts.domain = mechanism.value;
    let child = new SPFValidator(opts, this.DNS.count);
    let childRes = child.check_host(hostip, sender);
    let me = this;
    return new Promise((resolve, reject) => {
      childRes.then((res) => {
        let intRes = me.stringToResult(res);
        if(intRes > PASS && intRes < TEMPERROR) {
          resolve(NEUTRAL);
        }
        else {
          resolve(intRes);
